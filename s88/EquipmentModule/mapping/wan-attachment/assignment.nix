{
  ...
  candidateUplinkNameForTarget = target: let
    ...
    # When multiple candidates match the same key, prefer an indirect match
    # (e.g. explicit deployment host uplink matching via its upstream field)
    # over a direct name match (e.g. site uplink auto-generated by CPM from
    # a realization port reference).  Direct matches have the uplink name
    # equal to the search key; indirect matches do not.
    indirectMatches = builtins.filter (c: c != upstream) candidates;
  in
    if builtins.length candidates == 1
    then builtins.head candidates
    else if builtins.length indirectMatches == 1
    then builtins.head indirectMatches
    else if builtins.length indirectMatches > 1
    then builtins.head indirectMatches
    else null;
