...
let
  ...
  logicalNodeIdentityForTarget =
    ...
    runtimeContext.logicalNodeIdentityForUnit {
      ...
      file = "s88/CM/network/policy/wan-attachment.nix";
    };
  ...
  uplinksRaw =
    if !(deploymentHost ? uplinks) then
      ...
      throw ''
        s88/CM/network/policy/wan-attachment.nix: deployment host '${deploymentHostName}' has non-attr uplinks
  ...
  configuredWanUplinkName =
    if !hostHasUplinks then
      ...
      then
        ...
        throw ''
          s88/CM/network/policy/wan-attachment.nix: render host '${hostName}' has invalid wanUplink '${
      ...
      then
        ...
        throw ''
          s88/CM/network/policy/wan-attachment.nix: deployment host '${deploymentHostName}' has invalid wanUplink '${
  ...
  configuredWanGroupToUplink =
    if renderHostConfig ? wanGroupToUplink then
      if builtins.isAttrs renderHostConfig.wanGroupToUplink then
        ...
        throw ''
          s88/CM/network/policy/wan-attachment.nix: render host '${hostName}' has non-attr wanGroupToUplink
      ...
      if builtins.isAttrs deploymentHost.wanGroupToUplink then
        ...
        throw ''
          s88/CM/network/policy/wan-attachment.nix: deployment host '${deploymentHostName}' has non-attr wanGroupToUplink
  ...
  _validateConfiguredWanGroupToUplink = forceAll (
    map (
      ...
      if !builtins.isString uplinkName then
        throw ''
          s88/CM/network/policy/wan-attachment.nix: wanGroupToUplink entry '${wanGroupName}' on host '${hostName}' must map to a string uplink name
        ''
      else if !builtins.hasAttr uplinkName uplinksRaw then
        throw ''
          s88/CM/network/policy/wan-attachment.nix: wanGroupToUplink entry '${wanGroupName}' on host '${hostName}' references unknown uplink '${uplinkName}'
  ...
  upstreamNamesForWanGroup =
    ...
    if builtins.length upstreamNames <= 1 then
      ...
      throw ''
        s88/CM/network/policy/wan-attachment.nix: WAN group '${wanGroupName}' resolved to multiple upstream identities on host '${hostName}'
  ...
  _validateStrictWanRendering =
    if !hostHasUplinks || wanGroupNames == [ ] || missingWanGroupAssignments == [ ] then
      ...
      throw ''
        s88/CM/network/policy/wan-attachment.nix: strict rendering requires explicit WAN uplink assignment for host '${hostName}'
    ...
    let
      ...
      originalBridge =
        if uplink ? bridge && builtins.isString uplink.bridge then
          ...
          throw ''
            s88/CM/network/policy/wan-attachment.nix: uplink '${uplinkName}' assigned to WAN group '${wanGroupName}' is missing bridge
  ...
  renderedHostBridgeNameForAssignedUplink =
    ...
    if renderedNames == [ ] then
      ...
      throw ''
        s88/CM/network/policy/wan-attachment.nix: uplink '${uplinkName}' resolved to multiple rendered WAN bridges
  ...
  fabricUplinkName =
    if !hostHasUplinks then
      ...
      then
        ...
        throw ''
          s88/CM/network/policy/wan-attachment.nix: render host '${hostName}' has invalid fabricUplink '${
      ...
      then
        ...
        throw ''
          s88/CM/network/policy/wan-attachment.nix: deployment host '${deploymentHostName}' has invalid fabricUplink '${
    ...
    let
      originalBridge =
        if uplink ? bridge && builtins.isString uplink.bridge then
          ...
          throw ''
            s88/CM/network/policy/wan-attachment.nix: uplink '${uplinkName}' is missing bridge
