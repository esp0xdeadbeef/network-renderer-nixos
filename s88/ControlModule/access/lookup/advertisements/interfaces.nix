...
let
  ...
  sourceKindFor =
    ...
    if semanticInterface ? kind && builtins.isString semanticInterface.kind then
      ...
      containerIface ? connectivity && builtins.isString (containerIface.connectivity.sourceKind or null)
    then
      containerIface.connectivity.sourceKind
    else if
      runtimeIface ? connectivity && builtins.isString (runtimeIface.connectivity.sourceKind or null)
    then
  ...
  entryFor =
    ...
    let
      ...
      explicitSubnet4 =
        if builtins.isString (semanticInterface.subnet4 or null) then semanticInterface.subnet4 else null;
      explicitSubnet6 =
        if builtins.isString (semanticInterface.subnet6 or null) then semanticInterface.subnet6 else null;
    ...
    if !(builtins.isString rawName) || rawName == "" then
      ...
      let
        matches =
          ...
          ++ (lib.filter (iface: interfaceMatchesName iface rawName) (
