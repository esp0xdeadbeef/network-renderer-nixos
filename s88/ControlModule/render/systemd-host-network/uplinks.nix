...
let
  ...
  bridgeNetworkFor =
    ...
    let
      originalBridge =
        if builtins.isString (uplink.originalBridge or null) then uplink.originalBridge else uplink.bridge;
    in
    if builtins.hasAttr originalBridge common.bridgeNetworks then
      common.bridgeNetworks.${originalBridge}
    else
      ...
      (
        ...
        let
          ...
          directBridgeUplinks = lib.filter (
            uplinkName: (uplinks.${uplinkName}.mode or "") != "vlan"
          ) uplinksOnParent;
          directBridgeReference =
            if directBridgeUplinks == [ ] then null else uplinks.${builtins.head directBridgeUplinks};
          directBridgeAliasesEquivalent =
            directBridgeReference != null
            && builtins.all (
              uplinkName:
              builtins.removeAttrs uplinks.${uplinkName} [ "upstream" ]
              == builtins.removeAttrs directBridgeReference [ "upstream" ]
            ) directBridgeUplinks;
          _singleDirectBridge =
            if builtins.length directBridgeUplinks <= 1 || directBridgeAliasesEquivalent then
              true
            else
              throw ''
                s88/CM/network/render/systemd-host-network.nix: multiple non-vlan uplinks on parent '${parentIf}' are not supported
                logical aliases are permitted only when they have identical bridge and host-network realization
      ...
      in
      {
        ...
        value = {
          ...
          // lib.optionalAttrs ((uplink.mode or "") == "trunk" && transitNamesOnUplink != [ ]) {
            VLAN = map (
              transitName: "${uplink.bridge}.${toString transitBridges.${transitName}.vlan}"
