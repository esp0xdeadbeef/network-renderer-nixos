...
intent_path="$(nix eval --extra-experimental-features 'nix-command flakes' --impure --raw --expr 'toString (import ./vm-input-home.nix).intentPath')"
inventory_path="$(nix eval --extra-experimental-features 'nix-command flakes' --impure --raw --expr 'toString (import ./vm-input-home.nix).inventoryPath')"

tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/network-renderer-nixos-home.XXXXXX")"
...
if grep -qF "advertisement still defaults from renderer policy" "$tmp_dir/render.stderr"; then
  archive_generated_artifacts "home-network"
  dump_generated_artifacts
fi
