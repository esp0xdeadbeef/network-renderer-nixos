#!/usr/bin/env bash
set -euo pipefail

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
source "${repo_root}/tests/lib/test-common.sh"
source "${repo_root}/tests/lib/example-render-scan.sh"

result_json="$(mktemp)"
eval_stderr="$(mktemp)"
violations="$(mktemp)"
trap 'rm -f "${result_json}" "${eval_stderr}" "${violations}"' EXIT

labs_root="$(flake_input_path network-labs)"

check_box() {
  local label="$1"
  local box_name="$2"
  local intent_path="$3"
  local inventory_path="$4"

  nix_eval_json_or_fail \
    "downstream-selector-default-paths:${label}:${box_name}" \
    "${result_json}" \
    "${eval_stderr}" \
    env REPO_ROOT="${repo_root}" \
      BOX_NAME="${box_name}" \
      INTENT_PATH="${intent_path}" \
      INVENTORY_PATH="${inventory_path}" \
      nix eval \
      --extra-experimental-features 'nix-command flakes' \
      --impure --json \
      --file "${repo_root}/tests/nix/downstream-selector-default-paths.nix"

  if [[ "$(_jq -r '(.missingDefaultRoutes // []) | length' "${result_json}")" != "0" ]]; then
    {
      echo "!!!! ${label}:${box_name}: downstream selector access ingress can not reach policy default path"
      _jq -S '{missingDefaultRoutes}' "${result_json}"
    } >>"${violations}"
  fi
}

check_example() {
  local example_dir="$1"
  local tmp_dir="$2"
  local dry_json="${tmp_dir}/dry.json"

  render_example_dry_json "${example_dir}" "${tmp_dir}" "${dry_json}"
  while IFS= read -r box_name; do
    check_box \
      "$(basename "${example_dir}")" \
      "${box_name}" \
      "${example_dir}/intent.nix" \
      "${example_dir}/inventory-nixos.nix"
  done < <(_jq -r '.render.containers | keys[]' "${dry_json}")
}

for example_dir in \
  "${labs_root}/examples/single-wan" \
  "${labs_root}/examples/single-wan-with-nebula" \
  "${labs_root}/examples/overlay-east-west" \
  "${labs_root}/examples/s-router-overlay-dns-lane-policy"
do
  tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/network-renderer-nixos-downstream-paths.XXXXXX")"
  check_example "${example_dir}" "${tmp_dir}"
  rm -rf "${tmp_dir}"
done

hat_dir="${labs_root}/HAT/emulated-isp-residential-testnet"
if [[ -f "${hat_dir}/intent.nix" && -f "${hat_dir}/inventory-nixos.nix" ]]; then
  check_box \
    "HAT/emulated-isp-residential-testnet" \
    "s-router-nixos" \
    "${hat_dir}/intent.nix" \
    "${hat_dir}/inventory-nixos.nix"
else
  echo "!!!! downstream-selector-default-paths: missing HAT fixture ${hat_dir}" >>"${violations}"
fi

if [[ -s "${violations}" ]]; then
  cat "${violations}" >&2
  fail "!!!! downstream-selector-default-paths: access-policy nft allows must have matching default routes"
fi

pass "downstream-selector-default-paths"
