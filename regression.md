...
- state=runtime-checks | 2026-05-15 full-loop site-C DNS validation still failed after CPM emitted
  ...
  pending.
- state=runtime-checks | 2026-05-15 full-loop Hetzner toplevel evaluation failed after the
  ...
  still pending.
- state=runtime-checks | 2026-05-14 full-loop local and Hetzner rebuilds failed while evaluating
...
- state=runtime-checks | 2026-05-14 full-loop endpoint dual-stack validation failed on
...
- state=runtime-checks | 2026-05-14 full-loop branch DNS validation reached
  ...
  validation is still pending.
- state=runtime-checks | 2026-05-14 live hostile delegated IPv6 public-egress debugging found
  ...
  is still pending.
- state=runtime-checks | 2026-05-14 full-loop site-C DNS-over-overlay failed at
  ...
  `bash tests/test-dns-service-policy-routes.sh`; full lab validation is still
  pending.
- state=runtime-checks | 2026-05-14 live hostile delegated IPv6 public-egress debugging showed CPM now
  ...
  validation is still pending.
- state=runtime-checks | 2026-05-14 live return-path debugging showed Hetzner `c-router-nebula-core`
  ...
  validation is still pending.
- state=runtime-checks | Network pipeline contract audit found policy endpoint mapping still parsed
  ...
  validation is still pending.
- state=runtime-checks | Network pipeline contract audit found container policy-routing derived source
...
- state=runtime-checks | Network pipeline contract audit found container network interface view could
  ...
  still pending.
- state=runtime-checks | Network pipeline contract audit found container policy-routing classes still
  ...
  pending.
- state=runtime-checks | Downstream selectors now preserve explicit CPM `policyOnly` default routes
...
- state=runtime-checks | Policy-router ingress tables now include service/tenant routes required by
...
- state=runtime-checks | `tests/test-passing-fixtures.sh` passed on 2026-05-10 after the downstream
  selector and policy service-ingress route fixes.
- state=runtime-checks | ControlModule boundary is explicit: `s88/ControlModule` must not import or
  execute `s88/Unit`, `s88/EquipmentModule`, or `s88/Site` code.
- state=runtime-checks | Unit owns host/runtime selection: deciding which runtime units and containers
  belong to a requested deployment host is Unit-level work.
- state=runtime-checks | Equipment owns host equipment composition: deciding which ControlModules are
  required for a selected host, such as host networking, container runtime, and
  validation, is Equipment-level work.
- state=runtime-checks | ControlModules own deterministic rendering of the narrowed slice they are
  given. They must not ask broad host/site/unit questions to decide whether the
  work applies.
- state=runtime-checks | Boundary checks are covered by `tests/test-controlmodule-boundary.sh`,
...
- state=runtime-checks | Structural keyword leakage is covered by
  ...
  routing.
- state=runtime-checks | Clean renderer outputs must not contain warnings or alarms. This is covered by
  `tests/test-warning-alarm-contract.sh` and by the passing fixture runners.
- state=runtime-checks | Renderer warning/error behavior is not allowed to be silent:
...
- state=runtime-checks | DNS service rendering preserves explicit `dnsService.outgoingInterfaces` and
...
- state=runtime-checks | 2026-05-14 live hostile IPv4/DNS hotpatching produced concrete regression
...
- state=runtime-checks | Long logical realization port aliases are preserved into rendered container
...
- state=runtime-checks | The DNS outbound source-address correction is locally covered by
  `tests/test-dns-local-records.sh`, `tests/test-dual-wan-branch-overlay.sh`,
  and `tests/test-hostile-dns-east-west.sh`.
- state=runtime-checks | No live `s-router-test`, `s-router-test-clients`, or Hetzner runtime
  ...
  production-ready.
- state=runtime-checks | DNS service source binding and public resolver output leak guard are locally
...
- state=runtime-checks | Upstream-selector policy-only IPv4 default projection is locally covered by
  ...
  `core-nebula`.
- state=runtime-checks | 2026-05-13 live enumeration captured NixOS-rendered endpoint/router nft
...
- state=runtime-checks | 2026-05-13 fast visible-scope refresh captured
...
- state=runtime-checks | Runtime observability is a production requirement. The 2026-05-14 hostile
...
- state=runtime-checks | 2026-05-14 live `s-router-test` + Hetzner enumeration found hostile IPv4
...
- state=runtime-checks | Critical security regression: hostile DNS/egress must be treated as failed
...
- state=runtime-checks | 2026-05-14 later hostile tenant-origin probing refined the IPv4 blocker:
...
- state=runtime-checks | The same live probe separated host-origin access-router traffic from
...
- state=runtime-checks | After the temporary branch-side route fixes, branch packets reached
...
- state=runtime-checks | 2026-05-14 Hetzner live enumeration found the public WAN core works while
...
- state=runtime-checks | 2026-05-14 endpoint enumeration from `s-router-test-clients` showed
...
- state=runtime-checks | 2026-05-14 endpoint enumeration showed direct public DNS remains blocked from
  ...
  those contexts.
- state=runtime-checks | 2026-05-14 DNS service enumeration showed access routers are rendered with
...
- state=runtime-checks | 2026-05-14 Hetzner live DNS probe showed `c-router-access-client` and
...
- state=runtime-checks | 2026-05-14 00:58 UTC live refresh reproduced DNS failures on both
...
- state=runtime-checks | The same live packet proof showed the query is NATed correctly at
...
- state=runtime-checks | 2026-05-14 Hetzner `c-router-policy` rendered very broad repeated DNS and
...
- state=runtime-checks | `tests/test-nix-file-loc.sh` reports by layer and hard-fails only over
  500 LOC by default. Files over 250 LOC must state either
  `TEMPORARY OVER-LIMIT` or `ACCEPTED OVER-LIMIT`.
- state=runtime-checks | `s88/ControlModule/lookup/host-query/inventory.nix`: TEMPORARY OVER-LIMIT until 2026-05-17.
...
- state=runtime-checks | `s88/ControlModule/firewall/lookup/assumptions.nix`: TEMPORARY OVER-LIMIT until 2026-05-17.
...
- state=runtime-checks | `s88/ControlModule/render/dry-config-model.nix`: TEMPORARY OVER-LIMIT until 2026-05-17.
...
- state=runtime-checks | `s88/ControlModule/render/container-networks/policy-routing.nix`: TEMPORARY OVER-LIMIT until 2026-05-17.
...
- state=runtime-checks | `s88/ControlModule/render/containers/dns-services.nix`: TEMPORARY OVER-LIMIT until 2026-05-17.
...
- state=runtime-checks | `s88/ControlModule/alarm/isa18.nix`: ACCEPTED OVER-LIMIT FOR NOW.
...
- state=runtime-checks | `s88/ControlModule/render/containers/bgp-services.nix`: TEMPORARY OVER-LIMIT until 2026-05-17.
  Current responsibility: renders BGP container services.
  Suspected split: service config assembly vs validation.
- state=runtime-checks | `s88/ControlModule/render/containers/default.nix`: TEMPORARY OVER-LIMIT until 2026-05-17.
  Current responsibility: composes container render modules.
  Suspected split: service assembly vs module selection.
- state=runtime-checks | `s88/ControlModule/firewall/lookup/communication-contract.nix`: TEMPORARY OVER-LIMIT until 2026-05-17.
  Current responsibility: resolves firewall communication contracts.
  Suspected split: relation parsing vs endpoint mapping.
- state=runtime-checks | `s88/Unit/physical/realization-ports/inventory.nix`: TEMPORARY OVER-LIMIT until 2026-05-17.
...
- state=runtime-checks | `s88/ControlModule/firewall/policy/access.nix`: TEMPORARY OVER-LIMIT until 2026-05-17.
...
- state=runtime-checks | Compile `intent.nix` with the NixOS inventory used by the existing external
  example path. If the directory has `getResolvedInventory.nix`, use it before
  compiling.
- state=runtime-checks | Save and inspect the full `90-dry-config.json` through `jq -c .`.
- state=runtime-checks | Check output-owned evidence, not source-file assumptions:
...
- state=runtime-checks | Write a focused test only after the output inspection identifies the contract
  implied by the example name.

Current status:

- state=runtime-checks | Raw compact outputs were generated during the 2026-05-06 investigation for
  all runnable `network-labs` examples. This is temporary evidence only, not
  durable validation.
- state=runtime-checks | High-level counts were sampled, but full semantic review was NOT completed.
- state=runtime-checks | The output-focused test changes from the same investigation are provisional
  until the per-input review below is completed.

Per-input backlog:

- state=runtime-checks | `examples/single-wan`: NOT DONE. Test the baseline single-WAN output: one
  WAN uplink, default reachability, tenant access routing, policy path, and no
  extra overlay/BGP/VLAN/service behavior.
- state=runtime-checks | `examples/single-wan-any-to-any-fw`: NOT DONE. Test that the firewall output
...
- state=runtime-checks | `examples/single-wan-bgp`: NOT DONE. Test BGP service output for the simple
  single-WAN case: daemon enablement, AS/neighbor/network statements, and
  placement on the intended containers.
- state=runtime-checks | `examples/single-wan-direct-transit`: NOT DONE. Test direct transit output:
...
- state=runtime-checks | `examples/single-wan-ipv6-pd`: NOT DONE. Test IPv6-PD output: delegated
  prefix consumption, downstream RA/DHCPv6 behavior, and absence of renderer
  default-derived warning alarms.
- state=runtime-checks | `examples/single-wan-uplink-ebgp`: NOT DONE. Test eBGP uplink output:
  external neighbor configuration, route advertisement, and uplink interface
  placement.
- state=runtime-checks | `examples/single-wan-uplink-static-egress`: NOT DONE. Test static egress
  output: explicit default/static routes, gateway placement, and policy rules
  tied to the modeled uplink.
- state=runtime-checks | `examples/single-wan-vlan-trunk-lanes`: NOT DONE. Test VLAN trunk output:
  VLAN netdevs, bridge/network fragments, DHCP/RA behavior, and lane separation.
- state=runtime-checks | `examples/single-wan-with-nebula`: NOT DONE. Test overlay output without
...
- state=runtime-checks | `examples/single-wan-with-nebula-any-to-any-fw`: NOT DONE. Test that overlay
  plus any-to-any firewall output keeps tenant/overlay/WAN scoping explicit and
  does not emit broad unqualified accepts.
- state=runtime-checks | `examples/multi-wan`: NOT DONE. Test multi-WAN output: distinct uplinks,
  selector/policy behavior, per-uplink routes, and no collapsed single-WAN
  assumptions.
- state=runtime-checks | `examples/multi-wan-dedicated-lanes`: NOT DONE. Test dedicated-lane output:
  separate policy lanes per access/uplink combination and no cross-lane leakage.
- state=runtime-checks | `examples/multi-enterprise`: NOT DONE. Test enterprise/site disambiguation:
  duplicate logical node names across enterprises must render distinct
  containers, nodes, policies, and host fragments.
- state=runtime-checks | `examples/overlay-east-west`: NOT DONE. Test east-west overlay output:
  inter-site routes, overlay attachment interfaces, and firewall/policy rules
  for east-west traffic only.
- state=runtime-checks | `examples/priority-stability`: NOT DONE. Test deterministic ordering:
  firewall/routing/service priority output should remain stable and not depend
  on attrset traversal order.
- state=runtime-checks | `examples/ipv6-pd-downstream-delegation`: NOT DONE. Test downstream
...
- state=runtime-checks | `examples/dual-wan-branch-overlay`: NOT DONE. Test dual-WAN branch overlay
  output: WAN split, branch overlay routes, service forwarding, DNS policy, and
  no cross-uplink policy leakage.
- state=runtime-checks | `examples/dual-wan-branch-overlay-bgp`: NOT DONE. Test the same dual-WAN
  branch overlay shape plus BGP service output and route advertisement.
- state=runtime-checks | `examples/s-router-overlay-dns-lane-policy`: NOT DONE. Test the full
...
- state=runtime-checks | `examples/s-router-public-overlay-service`: NOT DONE. Test public overlay
  service output: DNAT/public ingress, runtime public address loading, overlay
  service firewall rules, and host policy routing.
- state=runtime-checks | `examples/tri-site-dual-wan-overlay-integration-static`: NOT DONE. Test
...
- state=runtime-checks | `examples/tri-site-dual-wan-overlay-integration-bgp`: NOT DONE. Test tri-site
...
- state=runtime-checks | Work through the network-labs output-analysis backlog one input at a time.
...
- state=runtime-checks | Before 2026-05-17, either split every `TEMPORARY OVER-LIMIT` file by the
  suspected responsibility split above, or replace the temporary marker with a
  real accepted-over-limit reason.
- state=runtime-checks | After local tests are green, run live validation for `s-router-test`,
